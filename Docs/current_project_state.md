...
- At the completion of phase 1, the new contract was behavior-neutral and was not connected to `engine.js` or PAGE1 lock transitions. Phase 2 activates only the state-preservation and `faceDetail` wardrobe boundaries described below; the remaining wardrobe, pose, and scene policies are still fixture-only.
