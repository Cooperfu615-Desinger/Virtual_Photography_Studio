...
- 共用 `poseHandId` 維持 22 個具體動作：`雙手自然垂放`、`雙臂交疊`、`一手扶腰一手自然放下`、`雙手背在身後`、`雙手放在頭後`、`雙手比 ROCK 手勢`、`單手向鏡頭張開手掌`、`單手托下巴`、`單手輕咬食指`、`單手往後撥瀏海`、`雙手抓著整束頭髮與髮尾整理`、`拉下上身服裝整理`、`雙手上拉上衣露出胸下緣`、`雙手遮住胸部`、`雙手拉開外套（雙肩滑落）`、`雙手把褲子或裙子的褲頭往上拉`、`雙手抱膝`、`雙手插褲子口袋`、`雙手插外套口袋`、`單手拿著眼鏡`、`單手把眼鏡拉下`、`咬著眼鏡腳`。這 22 個屬於共用手部清單，但各選項仍依姿勢基底限制顯示，不包含姿勢專屬手部池。`雙手抱膝` 是不綁定特定基底的下身手部動作，可與自然蹲姿、坐姿或其他可見下半身姿勢組合。
...
- 蹲姿的具體手部選項只保留六個專用動作：`雙手向前伸展`、`雙手自然放在兩腿外側`、`單手托腮一手扶膝`、`單手碰嘴角一手自然下垂`、`單手在臉旁比 V`、`雙手托腮扶臉`；蹲姿 UI 與隨機池不顯示或抽取 22 個共用手勢，但 `全無`、`隨機`、`任意` 三個控制項仍保留。其他姿勢依各選項的基底限制使用共用手勢。`雙手向前伸展` 的支撐點是分別倚靠左右膝蓋的雙肘，不是手掌；上半身前傾並讓手肘承接部分重量，前臂越過膝蓋向前下方伸展，手腕與雙手自然放鬆垂下，不再要求雙手在兩膝前靠攏。
