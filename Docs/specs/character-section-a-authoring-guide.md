...
站姿分類目前保留九個公開的肢體變化，將重疊的前傾、側傾、回身、抬腳與微彎膝選項設為 `uiHidden: true`、`randomEligible: false`。這些舊 ID 不刪除，仍可由既有 Saved Cards、匯入或明確 restore 還原；新 UI 與隨機流程不再選取它們。
...
站姿第一版專屬選項矩陣目前保留上述九個肢體變化。`骨盆後推曲線站姿` 只固定骨盆明顯後推、腰背形成清楚弧度、單腿承重與上身輕微前傾，不綁定人物朝向、頭部、手部、服裝或支撐物；環繞角度仍獨立決定觀看方向。此姿勢在牛仔中景、全身、固定構圖與未限制景別保留完整描述，在中景與更近景別省略，避免輸出畫面外臀髖幾何。站姿手部動作排除 `雙手抱膝`；拉下肩線、拉褲頭、褲子口袋、外套口袋與三個眼鏡動作依服裝／眼鏡條件顯示。站姿頭部方向排除 `頭部貼近支撐面`、`頭靠近邊緣支撐` 與 `近鏡頭偏轉頭部` 這類支撐或攝影機專用描述。`自然受支撐` 保留作為相容性選項，但不列入站姿一般選單或隨機池；站姿的主要接觸／支撐使用垂直面、髖側邊緣，以及選取水域場景後才可見的水中／水邊／浴缸選項。明確還原的舊鎖仍可輸出，不因這些站姿矩陣限制而被靜默改寫。
...
蹲姿公開英文已完成重寫：直接使用可接在 `a/an` 後面的視覺名詞片語，不使用 `arrangement` 或 renderer 控制語。七個公開蹲姿的完整描述都必須明確固定雙腳接觸支撐面、腳跟落地與臀部低位；自然、單膝不對稱、側身、前傾、雙膝合併半蹲與寬膝深蹲使用 `both feet planted flat`、`both heels grounded` 與 `hips held low over the feet` 的自然句型。`低蹲單腿前伸` 也要明確寫出雙腳平放與雙腳跟落地，再描述一腿向前伸直、另一腿折在身下，避免模型改成踮腳。`單膝抬起不對稱蹲姿` 必須寫出一側膝蓋較高與另一腿深屈；`側身蹲姿` 只寫臀腿維持低蹲、軀幹轉向鏡頭，不得使用會把整個人物變成側面的 `side-facing`；`身體前傾蹲姿` 必須寫出軀幹從髖部向大腿前傾；`雙膝合併半蹲` 使用 `half-squat` 與雙膝、雙腿平行的幾何；`寬膝深蹲／流氓蹲姿` 寫出雙腳寬距、膝蓋向外與低髖部。上半身裁切片段只能描述軀幹直立或前傾，不補造畫面外腳部接觸；下半身主導選項維持上裁切 `omit`。退役選項的英文不回寫，避免破壞舊 Saved Cards / restore 的歷史輸出。
...
- `雙手比愛心`（`both-hands-heart-near-face`）與 `手指愛心`（`one-hand-finger-heart`）是全姿勢可用的手動選項：站、坐、跪、蹲、仰躺、側躺與趴臥均可選擇，兩者皆 `randomEligible: false`。這是蹲姿／躺姿共用手勢限制的明確例外，不改變其專用隨機池。
- `雙手比愛心`：雙手在臉部下緣、下巴左側或右側合成一個愛心；拇指尖在下方相接，彎曲食指在上方相接，手腕與手肘自然放鬆。左右由出圖模型決定，不在程式端抽選，不綁定表情、頭部方向、服裝或拍攝方位。
- `手指愛心`：單手拇指與食指在指尖附近交叉，其餘手指自然收攏；可自然持在身前，也可伸向鏡頭形成放大的前景手指愛心。不固定左右手、距離或畫面佔比，另一隻手自然放置。
- 兩組愛心手勢使用明確 `projectionByBucket`：胸上到全身、未限制及固定構圖保留完整來源；`headShoulders` 只保留作者提供的手勢片段，移除畫面外手肘與另一隻手的描述；`faceDetail` 省略手勢但保留原 lock。此頭肩例外不恢復身體姿勢或其他手勢；仰躺原有表面來源仍保留。胸上衍生輸出重新取同一選擇的完整胸上手勢，全身角色參考照維持不帶動作。
...
- `自然自拍`、`鏡子自拍`、`男友/閨蜜自拍` 是三個全姿勢可用的公開手部例外：站、坐、跪、蹲、躺以及三種躺姿方向都可手動選擇。三者維持 `randomEligible: false`，不進入一般手部隨機池；選中後依 `selfie_hand_pose` 規則將環繞方向解析為 `全無`。

- 共用 `poseHandId` 維持 24 個具體動作：`雙手自然垂放`、`雙臂交疊`、`一手扶腰一手自然放下`、`雙手背在身後`、`雙手放在頭後`、`雙手比 ROCK 手勢`、`雙手比愛心`、`手指愛心`、`單手向鏡頭張開手掌`、`單手托下巴`、`單手輕咬食指`、`單手往後撥瀏海`、`雙手抓著整束頭髮與髮尾整理`、`拉下上身服裝整理`、`雙手上拉上衣露出胸下緣`、`雙手遮住胸部`、`雙手拉開外套（雙肩滑落）`、`雙手把褲子或裙子的褲頭往上拉`、`雙手抱膝`、`雙手插褲子口袋`、`雙手插外套口袋`、`單手拿著眼鏡`、`單手把眼鏡拉下`、`咬著眼鏡腳`。這 24 個屬於共用手部清單，但各選項仍依姿勢基底限制顯示，不包含姿勢專屬手部池。`雙手抱膝` 是不綁定特定基底的下身手部動作，可與自然蹲姿、坐姿或其他可見下半身姿勢組合。
- `單手向鏡頭張開手掌` 是構圖主導的近鏡頭遮擋動作，不是揮手或打招呼。動作不鎖定左手或右手；完整手掌必須位於鏡頭正前方並約佔畫面 80–90%，五根手指從指根到指尖完整可見、指尖靠近畫面邊緣但不得裁切，人物臉部、服裝與環境只在手掌外圍留下少量不完整片段。此選項除共用 projected canonical pose 外，另以同一份 option metadata 提供三個主要 renderer 共用且可追溯的 composition modifier；不得讓各 renderer 自行改寫成不同遮擋程度，也不得把 modifier 套用到其他手部動作、雙人模式或全身角色參考輸出。
- 蹲姿的具體手部選項只保留六個專用動作：`雙手向前伸展`、`雙手自然放在兩腿外側`、`單手托腮一手扶膝`、`單手碰嘴角一手自然下垂`、`單手在臉旁比 V`、`雙手托腮扶臉`；除上述兩組手動愛心例外外，蹲姿 UI 與隨機池不顯示或抽取其餘共用手勢，但 `全無`、`隨機`、`任意` 三個控制項仍保留。其他姿勢依各選項的基底限制使用共用手勢。`雙手向前伸展` 的支撐點是分別倚靠左右膝蓋的雙肘，不是手掌；上半身前傾並讓手肘承接部分重量，前臂越過膝蓋向前下方伸展，手腕與雙手自然放鬆垂下，不再要求雙手在兩膝前靠攏。
- 躺姿手部依 `仰躺`、`側躺`、`趴臥` 分成各自的專用池，並由 `visibleBuckets`、方向矩陣與隨機相容性規則控制；不得把躺姿專用動作誤列為共用手部。
- 上述蹲姿六組限制只指普通手勢；三個手動自拍例外會另外顯示在蹲姿 UI，但不參與蹲姿隨機池。三個自拍也不屬於躺姿專用隨機池，但在仰躺、側躺與趴臥 UI 中仍可選擇。
- `雙手抓著整束頭髮與髮尾整理` 必須明確描述一手抓住髮根附近、另一手抓住並順過整束髮尾的自然整理動作，不得只寫籠統的 `arrange hair`。
- `單手往後撥瀏海` 必須描述手指把瀏海向後梳理的帥氣整理動作，不再使用含糊的 `brush hair`。
- `拉下上身服裝整理` 描述單手輕輕將已穿著的上身服裝向下拉並整理，讓衣料自然回位；不指定衣物部位、肩膀是否露出或服裝款式。沿用原 `hand-adjust-off-shoulder-top` ID，舊英文僅保留在 `legacyPromptAliases` 作 Markdown 匯入解析。
- `雙手上拉上衣露出胸下緣` 是獨立的手動選項：雙手抓住上身服裝前側下擺的左右兩側，從腰部上拉到肋骨上緣，衣料停在胸部下三分之一處，胸下緣可見而胸部上方仍遮住，服裝保持穿著。使用者須選擇適合上拉的上身服裝；不進入隨機池，不綁定服裝款式、頭部、神情或拍攝角度。限站、坐、跪姿；腰上、牛仔中景、全身與固定構圖保留動作，胸上及更緊的裁切省略動作文字但保留原選擇。
- `雙手遮住胸部` 是新增的獨立手動選項，使用新 `hands-cover-breasts` ID：雙手將上身服裝前側拉至胸部上方，衣料聚攏在胸部上緣，雙手各遮住一側乳頭，周圍胸部仍可見，服裝保持穿在肩上。沿用上身服裝存在條件，但不綁定款式、顏色、頭部、表情或拍攝角度；不進入隨機池。限站、坐、跪姿；胸上、腰上、牛仔中景、全身與固定構圖保留動作，頭肩及更緊的裁切省略動作文字但保留原選擇。
- `雙手拉開外套（雙肩滑落）` 是獨立的手動選項：雙手在胸口附近抓緊已敞開外套的前緣並向外大幅拉開，同時外套領口和肩線滑到雙側上臂，雙臂仍留在袖內，前片形成兩片寬大的敞開布面。動作來源只描述外套與手臂的互動，不綁定外套款式、顏色、內搭、人物或場景。未明確指定外套開合／穿法時，這個動作使已選外套解析為 `敞開穿` 與 `雙肩露出`；明確鎖定的開合／穿法仍保留。限站、坐、跪姿，不進入隨機手部池；胸上至全身與固定構圖保留動作，頭肩及更緊的裁切省略動作文字但保留原選擇。
- `雙手把褲子或裙子的褲頭往上拉` 的公開 Prompt 必須使用服裝類型中性的正向說法：雙手輕抓腰部兩側衣料，小幅向上調整至自然位置。不得在新公開文字中並列 `pants or skirt`、虛構 `belt loops`，或重複往下拉／脫下的否定概念；舊文字只作 `legacyPromptAliases` 解析。
- `單手拿著眼鏡`、`單手把眼鏡拉下`、`咬著眼鏡腳` 都要求眼鏡是已存在的穿戴配件；其中咬眼鏡腳要寫明鏡腳輕放齒間、鏡框取下後自然垂在臉頰旁。
- 服裝／配件條件由手部 option metadata 與 shared random compatibility resolver 同步執行：上身服裝整理需有明確選定的上衣、洋裝、外套、套裝或特殊穿搭；褲頭／褲袋需有褲或裙，外套口袋需有外套，眼鏡互動需有眼鏡。只有 required role 明確解析為 `present` 時才可進入隨機池，`absent`、`unknown` 或隨機尚未解析都會排除，以免公開 prompt 虛構不存在的服裝／配件。這只限制隨機候選，不覆寫使用者明確手勢 lock。
...
躺姿現在採用兩層結構：`poseOrientationId` 提供三個主要方向 `仰躺`、`側躺`、`趴臥`，`poseArrangementId` 提供九個獨立的身體／腿部變化：`自然伸展`、`雙腿屈起`、`身體微蜷`、`上半身半躺`、`上半身撐起`、`下半身側轉・雙腿交疊`、`下半身側轉・雙腿分開`、`單腿交疊伸展`、`雙腿屈起交疊`。主要躺姿不綁定鳥瞰鏡頭；鏡頭仍由攝影設定獨立控制。`自然伸展`、`雙腿屈起`、`身體微蜷` 可用於三種方向；`上半身半躺` 只進入仰躺；`上半身撐起` 只進入側躺與趴臥；後四個腿部變化只進入仰躺。`自然伸展`、`雙腿屈起` 與後四個仰躺腿部變化屬於下半身幾何，在胸上／腰上裁切應 `omit`；`身體微蜷`、`上半身半躺` 與 `上半身撐起` 提供可見的上半身投影片段，其中 `身體微蜷` 必須明確表達膝蓋靠近胸口與脊椎圓弧的大幅度緊密蜷曲。全身、unconstrained 與 fixed composition 保留方向與身體變化的完整 canonical pose。舊躺姿專用 arrangement ID 保留但設為 `uiHidden: true`、`randomEligible: false`，供 Saved Cards、匯入與明確 restore 還原。

躺姿公開英文使用直接、正向的視覺片語：側躺與趴臥仍使用 `side-lying position, torso turned onto one side`、`prone position, torso facing downward`；仰躺改用表面主導的自然句型，不再輸出 `supine position` 或其他方向控制語，改由 `She lies on ... that dominates the composition` 表達「她躺在主要支撐面上」。方向片語只描述軀幹朝向，不自行指定臉部方向或具體支撐物；頭部、手部與接觸／支撐由各自獨立選項負責。身體／腿部變化使用 `her body resting in a relaxed extended line with the legs resting naturally`、`both of her knees comfortably bent and resting naturally`、`her body deeply curled inward, her knees drawn close toward her chest and her spine rounded into a tight compact shape`、`her upper body slightly raised in a gentle half-recline`、`upper body raised into a supported incline`，以及四個仰躺專用的下半身幾何片語；只描述身體幾何，不加入鏡頭、視線或內部控制語。`身體微蜷` 必須表達膝蓋靠近胸口、脊椎圓弧與緊密收縮，避免只寫成輕微內縮。仰躺手部保留「雙手自然放在身側、一手放在頭後、一手放在腹部、雙手向頭頂伸展、雙手合掌靠在臉側」；其中 `仰躺雙手向頭頂伸展` 在 Chest-up／Medium 使用專屬投影片段，保留雙臂向頭頂延伸，但允許前臂或手自然延伸到畫框上緣外，不強制完整手掌入鏡；全身、Cowboy、unconstrained 與 fixed composition 仍使用完整手勢來源。側躺與趴臥各使用自己的下側手臂／身前／臉側／肘部支撐選項。共用手部與頭部選項在躺姿隨機池停用，但舊 ID 仍可明確還原。仰躺是特殊的表面主導模式：PAGE1 UI 將 `固定構圖場景`、`場景基底`、PAGE3 空景架構與其匯入／可見文字欄位顯示為 `全無` 並停用；生成 prompt 與 selection/export 同樣輸出 `none`，但公共 prompt 不輸出 `none`、`全無` 或場景控制語。只有 `環境光線` 保持可操作。仰躺的 `接觸／支撐` 選項改為獨立表面池：床、榻榻米、地板、乾浴缸、沙發、草地、水泥地、白色細沙沙灘與清澈海面，另保留水中／水邊支撐；每個表面可附一個簡短生活感環境句，並由支撐面主導構圖。所有帶有 `supineSurfaceLed` 的仰躺表面在任何 composition visibility bucket 都必須保留完整表面身份與環境句；Face Detail／Head Shoulders 雖維持一般姿勢幾何省略，仍不得移除表面來源，Chest-up／Medium 也不得將表面縮短成不具身份的通用支撐語。`在水中` 必須明確描述人物仰漂在清澈水面、身體部分浸水，以及皮膚、衣物與頭髮的濕潤狀態；`海面上` 必須描述人物仰漂於清澈的土耳其藍淺海、身體部分浸水且濕潤，並保留水面漣漪與水下珊瑚礁。水泥地使用水泥沙包、木材與鋼材等工地元素，榻榻米使用隨意擺放的日式坐墊與日本雜誌。切離仰躺回站、坐、蹲、跪、側躺或趴臥時，UI 會恢復切換前的場景／固定構圖值。Chest-up／Medium projection 仍只輸出畫面可見的上半身片段；下半身變化不回寫到胸上／腰上裁切。
