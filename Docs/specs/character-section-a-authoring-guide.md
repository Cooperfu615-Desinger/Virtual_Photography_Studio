...
躺姿公開英文使用直接、正向的視覺片語：`supine position, torso facing upward`、`side-lying position, torso turned onto one side`、`prone position, torso facing downward`。方向片語只描述軀幹朝向，不自行指定臉部方向或具體支撐物；頭部、手部與接觸／支撐由各自獨立選項負責。身體／腿部變化使用 `body extended in a relaxed line with the legs resting naturally`、`both legs comfortably bent with the knees softly raised`、`torso and legs gently curled inward into a compact shape`、`upper body raised into a gentle half-recline`、`upper body raised into a supported incline`，只描述身體幾何，不加入鏡頭、視線、環境或獨立手部動作。仰躺手部保留「雙手自然放在身側、一手放在頭後、一手放在腹部、雙手向頭頂伸展、雙手合掌靠在臉側」；側躺與趴臥各使用自己的下側手臂／身前／臉側／肘部支撐選項。共用手部與頭部選項在躺姿隨機池停用，但舊 ID 仍可明確還原。接觸／支撐改由躺姿專用的床、榻榻米、地板、乾浴缸、沙發、草地、水泥地與沙地選項提供，英文以 `on ... surface`、`in an empty dry bathtub, supported by its interior surface` 或 `on ... ground` 描述，並依 `室內`／`戶外` 場景屬性顯示；`在水中` 與 `靠在水邊支撐` 仍須水域場景。Chest-up／Medium projection 使用只含畫面可見上半身的片段；下半身變化不回寫到胸上／腰上裁切。
