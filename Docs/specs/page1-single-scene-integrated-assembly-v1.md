...
- `meta.preserveInSceneIntegratedZ` 明確標記的具體承重物件保留在 Z-Image 姿勢句中。第一版只標記 `坐在單人雕花絨布椅`，因此輸出仍包含 `on an ornate single velvet armchair in a relaxed lounging posture`；不依英文關鍵字推測其他 anchor。
