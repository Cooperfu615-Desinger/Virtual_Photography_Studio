...
| 輸出／表面 | 第一版規則 |
| --- | --- |
| `Z-Image` → `zImagePrompt` | 調整組裝順序與來源連接；省略關係型接觸／支撐，保留明確標記的具體承重物件；保留有效攝影風格與成像來源 |
...
| 歷史契約 | 已接入的窄例外 |
| --- | --- |
| 根目錄 AGENTS、主撰寫規範與 Z-Image contract 要求三版逐字共用 canonical pose | 只允許目標 Z-Image 從同一投影後姿勢來源省略關係型 anchor；明確標記的具體承重物件仍保留。GPT／MJ 與共用 canonical 保持原樣 |
...
4. **姿勢**：保留投影後有效的基底／朝向、肢體變化、手部／道具、頭部方向。省略關係型獨立 anchor，但保留明確標記的具體承重物件，詳見第 5 節。
...
使用者確認的是：**一般關係型接觸／支撐可以省略，但不能因此刪掉決定畫面必要家具的具體承重物件，也不是移除所有提及接觸的姿勢動作。**

- 預設省略 `poseAnchorId` 所擁有的關係型姿勢片段，不清空該 lock、selection、摘要或 Saved Cards 記錄。
- `meta.preserveInSceneIntegratedZ` 明確標記的手動坐姿物件保留在 Z-Image 姿勢句中。第一版物件為一般椅子、單人雕花絨布椅、沙發、長椅、台階、床、地面、浴缸與洗手台旁浴室檯面；輸出只保留簡潔的「坐姿＋物件」來源，例如 `in an ornate single velvet armchair`，不加入承重、接觸清楚可見或其他推導，也不依英文關鍵字猜測未標記 anchor。
- 站姿、跪姿與蹲姿的公開接觸／支撐選項只保留 `全無`，其語意是未指定外部物件、由模型依姿勢自由安排；隨機池不再抽取關係型或具體外部支撐。既有 ID 仍可由舊資料還原，但不回到公開選單或隨機池。
- 坐姿的九個具體物件只供使用者手動選擇，不進入隨機池。`全無` 同樣表示未指定外部物件；不因自由安排而自動補椅子、牆面、柱子或欄杆。
