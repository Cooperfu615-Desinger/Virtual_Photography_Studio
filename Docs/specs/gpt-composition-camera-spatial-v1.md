...
2026-09-18 距離語意補正：高位俯視、鳥瞰、正上方三組加入與 Z-Image 共用的軟性相機—人物距離提示；不新增 UI、lock、焦段或實體相機控制。
...
- 高位俯視全身：以約 1.5–2 公尺的人像距離維持人物主導的斜向俯拍；非站姿使用一般近遠身體區域，不強制頭肩最近。鳥瞰：約 3–5 公尺的高處並向後拉開，斜向俯看人物與空間。正上方：約 1–2 公尺的正上方位置，以 90 度垂直頂視完整人物。
- 非全身同樣保留所選裁切：高位使用較近的人像距離，鳥瞰使用較遠的空間距離並保留所選裁切，正上方使用垂直 90 度與扁平構圖；不要求地面入鏡或擴大成全身。平視／肩／腰／膝／荷蘭角的非全身短句保持原樣。
- 距離只作為自然語言的軟性尺度提示，不是 UI 或硬性相機參數；不綁魚眼或改焦段，不新增自拍衝突規則，極端視角／焦段／自拍組合仍需使用者實測。本次沒有自動相容性修正。

## 實作與驗證設計

- `gptCameraSpatial.js` v1.2.0：共用已確認的 angle-only 語意與全身六組，並為高位／鳥瞰／正上方加入與 Z-Image 一致的軟性距離提示；不取得 Z 完整輸出或 orbit 語意。
