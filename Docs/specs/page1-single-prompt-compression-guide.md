...
2026-09-14 既有細節優先：契約 `1.11.0` 僅一般單人主 Z 的七組核准場景、有效低機位，選用同場景原始來源中的指定短片語替代既有優先細節；三組對照維持原樣。這是以下「不回補原始來源」的有限例外，不恢復完整 catalog 或泛用家具。原始身份／片語與目前前綴不符時不套用；資料庫、其他 renderer 與十八組新增來源不改。見 [正式規則](z-image-existing-scene-detail-priority-v1.md)。
