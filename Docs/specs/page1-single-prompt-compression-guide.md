...
2026-09-22 上身圖案修正：主 MJ 與 MJ 胸上必須保留共用投影上衣中已選的圖案片段，不能在衣物精簡時丟掉。近景主景別也會解析明確選定的上身圖案，供各衍生輸出沿用；不新增抽選、不改儲存欄位。Public output contract `1.18.0`、MJ description contract `1.9.0`；具體來源與排除規則見 [穿搭撰寫規範 §9.2](wardrobe-section-b-authoring-guide.md#92-圖案)。
...
2026-09-15 既有場景細節優先：一般單人主 Z 的低機位優先來源已從 v1 的 7 組修改擴展為 45 個 runtime location ID（7 組歷史修改加 38 組新增；3 組對照僅作 fixtures）。完整清單與 456 筆整合矩陣見 [既有細節優先 v2](z-image-existing-scene-detail-priority-v2.md)；GPT、MJ、雙人、固定構圖、角色卡、仰躺與其他排除路徑不變。
...
2026-09-14 環境光共同描述：36 組已核准的環境核心＋條件性細節，僅用於一般單人主 GPT/Z。GPT 完整保留新版有效描述，不套用 Z 的高度刪除；Z 不再固定取前兩片語，僅投影已標記天空／表面細節。該環境光批次不改動 25 組人物受光；目前人物受光來源與邊界以 [主體光線 v1](subject-light-styles-v1.md) 為準。這是 GPT 原始英文保留規則的限定來源改寫，見 [正式規格與英文](ambient-light-shared-descriptions-v1.md)。

2026-09-14 既有細節優先（v1 歷史批次）：契約 `1.11.0` 僅一般單人主 Z 的七組核准場景、有效低機位，選用同場景原始來源中的指定短片語替代既有優先細節；三組對照維持原樣。這是以下「不回補原始來源」的有限例外，不恢復完整 catalog 或泛用家具。完整目前範圍見 [既有細節優先 v2](z-image-existing-scene-detail-priority-v2.md)。

2026-09-13 上方場景擴充：契約 `1.10.0`／補充來源 `1.1.0` 已納入全部十八個核准 location ID（首批四個＋十四個擴充）；仍只作用於既有一般單人主 Z 低機位路徑。其餘生成、裁切、UI 與資料庫契約不變，正式文字與驗證見 [v2 規格](z-image-upper-scene-v2.md)。下方首批版本號保留作歷史脈絡。

2026-09-13 Z-Image 全身相機呈現：契約 `1.9.0` 僅一般單人主 Z 的 fullBody angle 句採用六組呈現方向；自然全身保留高度短句，荷蘭角保留畫面傾斜，地面與貼地近距離蟲眼分開。其他景別、GPT/MJ、衍生與排除路徑不變；不改所選焦段、姿勢、UI 或資料庫。場景地面／天空規則仍讀原始有效 angle。這是下文既有全身 angle 文案的窄例外，見 [全身相機 v1](z-image-full-body-camera-v1.md)。

2026-09-13 Z-Image 上方場景首批：契約 `1.8.0` 在第一版方向刪減後，僅對四個已核准 location ID、有效腰部／膝蓋／地面／蟲眼角度追加已撰寫的一至兩個短片語；不是回讀原資料庫或推測上方物件。其他輸出與未列場景不變。這是以下「不新增上方造景」歷史敘述的明確窄例外，見 [v2 首批規格](z-image-upper-scene-v2.md)。
...
| `半臉傾斜特寫`、`全臉傾斜特寫`、`特寫鏡頭 (Close-Up)` | `headShoulders` | 上衣／連身服的領口與肩部、外套肩部、頭頸配件 | 完全省略 canonical pose | 原場景來源可追溯地壓成一句，不輸出互動幾何或支撐物 |
...
| `半臉傾斜特寫`、`全臉傾斜特寫`、`特寫鏡頭 (Close-Up)` | `headShoulders` | 完全省略 Body Type 與 Character Card `body`；非體態身份資料不受影響。 |
...
主 Prompt 的現行選單與隨機池使用 `半臉傾斜特寫`、`全臉傾斜特寫`、`中景鏡頭 (Medium Shot)`、`牛仔中景 (Cowboy Shot)`、`全身鏡頭 (Full Body Shot)`，並保留可明確選取的 `全無`。`局部五官特寫`、`臉部特寫`、`特寫鏡頭 (Close-Up)`、`胸上特寫` 的既有 option ID 不刪除或改名；舊 Saved Cards、瀏覽器儲存與 restore payload 仍可解析並保存原值。還原舊值時，PAGE1 暫時顯示該值為 disabled restore-only option，使用者改選新版景別後即不再出現在選單，新隨機結果也不會抽中舊值。
...
`全臉傾斜特寫` 與半臉選項並存且使用獨立 option ID。它同樣在每次生成中只解析一次左側或右側位置並由三個主要 renderer 共用；人物須靠近指定側邊、整張臉與臉部輪廓完整留在畫面內、對側保留大面積負空間，並明確禁止置中。全臉版本使用 `headShoulders` 投影及輕微傾斜畫面，但不使用半臉的 `partial_face` 方位限制。
