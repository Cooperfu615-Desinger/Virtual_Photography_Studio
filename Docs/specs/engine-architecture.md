...
| `webapp/src/lib/engine/aiPromptLengthContract.js` | Historical single-subject AI word diagnostics, immutable sections, preservation anchors, and compatibility policy |
...
| `webapp/src/lib/engine/aiPromptLengthIntegration.test.js` | Blocking gate for historical mappings, contracts, resolved selections, anchors, MJ diagnostics, and canonical pose |
...
| `webapp/src/lib/engine/fixedFramingDerivedPromptContract.js` | Target contract for derived outputs, simplified main-framing visibility, legacy ID preservation, and half/full-face edge placement |
...
Midjourney V8 parameter phase 6 is a behavior-neutral completion gate. `midjourneyCompletionGate.test.js` regenerates all seven deterministic parameter fixtures and blocks drift across the engine, all six public Prompt contracts, PAGE1 generation cards, DLL Prompt sources, Standard Prompt import, Favorites v3 serialization, and Saved Cards Markdown import. It verifies the historical field mappings, exact Gpt／Grok-Z baselines, phase-5 AI description hashes, canonical parameter tails, resolved F／aspect-ratio selections, parameter-free derived outputs, and exact canonical-pose reuse. The gate is included in `npm run test:prompt-quality`; phase 6 does not modify renderer text, UI behavior, storage IDs, or public field names.

Midjourney V8 parameter phase 7 adds the independent `mjAspectRatio` F control. Its enum exposes common Midjourney integer ratios plus a `page1` fallback; explicit values affect only the AI parameter tail and remain outside the descriptive-content budget, while `page1` continues to project the resolved legacy `selection.aspectRatio`. `createPromptGenerationLocks()` excludes the new key from rerolls and global clear/random actions, and `appendMidjourneyParameterTail()` emits exactly one `--ar` in the existing canonical order. Standard Prompt and Saved Cards import restore the new F value while retaining the legacy PAGE1 ratio field for compatibility. No other renderer, public field mapping, storage ID, canonical pose, or descriptive source wording changes in this phase.

The F UI presents that discrete ratio enum through the same current-value plus range-slider interaction as Stylize, Chaos, and Weirdness; the slider maps positions to existing option IDs and does not introduce arbitrary ratio text input. Contract-owned `parameterHelp` data drives the UI's per-parameter tail explanation, including the directional meaning of larger and smaller numeric values, without changing the emitted tail or any public Prompt output.

Midjourney-native description optimization phase 1 starts a separate six-phase contract after the parameter work is complete. `midjourneyDescriptionContract.js` defines one single-line sequence of image type, composition, subject, wardrobe, projected canonical pose, scene／lighting, and imaging before the independently owned parameter tail. Semantic groups use sentence boundaries and internal source clauses use commas. The target forbids imperative `Create a／an` openings from phase 2 onward, preserves one resolved selection, prohibits invented visual facts, and retains exact canonical-pose reuse plus all historical public mappings. `midjourneyDescriptionFixtures.js` records seven byte-stable phase-1 description hashes and direct-opening targets for all six image types. Phase 1 changes no renderer or public Prompt text.

Midjourney-native description optimization phase 2 activates direct image-type openings only inside `midjourneyPrompt`. `buildMidjourneyImageTypePromptLine()` reads the contract-owned identity for all six image types, while Gpt and Grok/Z-Image continue using their original image-type sentences. The AI composition producer now owns its terminal period, preventing composition and subject text from collapsing into `front view A 20s...`. Single and duo AI use the same opening boundary; the public AI output contract and strict audit accept only approved direct openings and normalize their identity against the longer Gpt／Grok-Z source sentence. The phase does not rewrite subject, wardrobe, pose, scene, lighting, imaging, or the parameter tail.

Midjourney-native description optimization phase 3 separates the normal single-subject identity lead from the historical cross-model subject constant. `buildAiFreedomSubjectSentence()` uses a direct Midjourney identity noun phrase while retaining the existing projected Body Type, hair, eyewear, and audio-accessory sources; Gpt and Grok/Z-Image keep their existing renderers and wording. The wardrobe producer continues to consume the shared composition visibility projection, then removes repeated lower-crop boundary phrases inside the AI wardrobe sentence while retaining the final boundary. Character Card, duo, complete-look, special-outfit, and dress-specific restructuring remains a phase-5 concern.

Midjourney-native description optimization phase 4 keeps `buildAiFreedomPoseSentence()` as an exact pass-through of the shared projected canonical pose. The scene producer emits its projected location as a direct sentence, followed by a separate direct lighting sentence from the same ambient and subject-light selections. The imaging producer converts the selected photographer source into `<name>-inspired <image language>` and removes only the `shot on` connector from the selected lens phrase; optical and film sources keep their existing compact clauses. No renderer outside `midjourneyPrompt` consumes these transformations.

Midjourney-native description optimization phase 5 applies the direct structure to the remaining AI modes. Character Card subjects keep all four permanent identity anchors and source-derived hair／body／accessory anchors while renderer-oriented hair-edit instructions are reduced to their visible result. Complete-look wardrobe fragments remove only a redundant terminal `outfit`. Duo AI is rendered as direct `First woman`／`Second woman` sentences with role-bound wardrobe and direct pose／scene／lighting／imaging sentences; all inline section labels are forbidden, while Gpt and Grok/Z-Image retain their historical labeled duo renderers. The length contract now includes a separate duo policy (160 target／180 soft max). Fixed-composition AI continues to consume the shared wardrobe and canonical-pose projection before its source-traceable fixed-set scene.

Midjourney-native description optimization phase 6 is behavior-neutral. The description contract records the required engine, public-contract, PAGE1／DLL, Standard Prompt, Favorites v3, and Saved Cards Markdown consumers together with the historical field mappings. `midjourneyCompletionGate.test.js` regenerates all seven accepted description targets and blocks drift in direct openings, one-block syntax, forbidden AI labels, phase-5 hashes, mode-specific soft limits, canonical pose, parameter-tail ownership, and downstream byte identity. No production renderer, selection, storage key, option ID, UI, or parameter setting changes in this phase.

### Implemented AI/Midjourney source-retention policy

The implemented behavior change is limited to the historical `AI` → `midjourneyPrompt` renderer. The focused regression fixture is `normal-single-ai-compression-source-retention`. The shared resolved selection, selection snapshot, storage/import compatibility, Gpt (`grokPrompt`), Grok/Z-Image (`zImagePrompt`), and canonical Pose Composer text remain unchanged.

- The AI composition opening is a source-derived immutable anchor. A medium, high-angle, rear-right selection must remain `Waist-up portrait, high angle, looking down, rear-right three-quarter view.`; it must not be generalized to `medium shot` or `high right-rear angle`.
- Normal single-subject AI retains the fixed identity sentence `A 20s seductive stunning Japanese woman.`, projected body anchors, one compact facial-feature anchor, and the selected expression/gaze source. Character Card uses its permanent identity anchors instead of appending a duplicate normal face clause; special subjects and duo retain their own identity contracts. Half-face composition wording is outside this change and remains unchanged.
- Scene selection is coverage-based and source-traceable: preserve location identity, a representative physical anchor, and selected time/weather/light conditions before removing secondary clauses. It must not add unselected depth, blur, bokeh, or scene objects.
- Imaging selection preserves photographer/style identity, lens identity, and the selected optical effect. Secondary `Camera / Film` explanation is reduced first; a distinct film/rendering identity remains when it is an authored source anchor.
- Subject owns hairstyle and hair color; Wardrobe owns garment, material, silhouette, color, footwear, and accessories. AI semantic deduplication may remove repeated ownership and internal-control language, but may not remove the primary garment, material, silhouette, or hairstyle identity. A special outfit may retain an explicitly authored hairstyle once.
- The project’s 1000-English-character target is an internal diagnostic or design target, not an official Midjourney limit. Historical AI word budgets remain measurable diagnostics only; the MJ renderer does not use them to delete resolved wardrobe or accessory items.

The expected direct single-subject shape is documented in `Docs/specs/page1-single-prompt-compression-guide.md`; this architecture note does not authorize a renderer rewrite, new selection resolver, consumer reassembly, or a change to public output field names.

AI length optimization phase 1 was behavior-neutral. It recorded separate normal, complete-look, and Character Card word budgets, plus the immutable image-type, composition, and projected canonical-pose sections. Deterministic fixtures captured the longest known outfit-preset, special-outfit, Character Card, pose, and crop pressure without changing `renderAiPrompt()`. Midjourney description optimization phase 5 later superseded the original duo exclusion with an explicit 160／180-word duo policy; hard string／word truncation remains prohibited.
...
AI length optimization phase 5 is retained as historical section-model behavior and diagnostics. Its old soft-max arbitration is no longer used by the MJ renderer after the full-retention policy was adopted; the standalone `aiPromptBudget.js` helpers remain available for measurement and compatibility tests.

AI length optimization phase 6 adds the blocking integration gate. The gate generates all ten deterministic length fixtures through `generatePrompts()`, validates `grokPrompt`, `zImagePrompt`, and `midjourneyPrompt` against their historical public contracts, verifies compatible resolved selections, required AI anchors, diagnostic word measurements, and exact canonical-pose reuse, and records the immutable-section boundaries. `npm run test:prompt-quality` includes this gate, so a future renderer, option, Character Card, crop, or output-mapping change cannot bypass the MJ retention policy silently.

The current MJ full-retention implementation keeps all effective visible wardrobe and accessory roles, including multiple complete-look accessory fragments, normal single-subject earrings／neck accessories, generic head accessories, and role-bound duo accessories. It first compacts each item into a short source-derived phrase, removes redundant styling tails, and only applies the existing composition-visibility projection to omit items outside the active crop. The renderer assembles the full section model without applying the historical word-budget arbitration; Gpt, Grok/Z-Image, storage, import／restore, UI IDs, and public field mappings remain unchanged.
...
Phase 5 activates the main-framing policy through `engine/fixedFramingMainPrompt.js`. PAGE1 exposes `全無`, half-face, full-face, medium, cowboy, and full-body options; unlocked engine resolution samples the five concrete candidates. The four retired close-range options stay in the complete engine catalog for normalization, Saved Cards, browser storage, and restore compatibility. A currently restored legacy value is projected into PAGE1 as a disabled restore-only option, then disappears after the user selects a current option. Half-face and full-face generation each resolve one seeded left- or right-edge opening once in the shared context, so Gpt, Grok/Z-Image, and AI receive identical geometry. Both use the `headShoulders` visibility bucket. Half-face crops through the outer half of the face; full-face keeps the entire facial outline inside the frame, requires the subject at the selected edge with opposite negative space, and explicitly forbids centered placement.
...
The phase-5 main-framing selector exposes `全無`, `半臉傾斜特寫`, `全臉傾斜特寫`, `中景鏡頭 (Medium Shot)`, `牛仔中景 (Cowboy Shot)`, and `全身鏡頭 (Full Body Shot)`. The existing IDs for `局部五官特寫`, `臉部特寫`, `特寫鏡頭 (Close-Up)`, and `胸上特寫` remain catalog-resolvable and restorable but are excluded from new UI selection and random candidates. The half-face and full-face targets each resolve one seeded placement, left or right, once per generated result and share the exact opening across Gpt, Grok/Z-Image, and AI. Half-face crops the outer half of the face at the chosen edge. Full-face retains a narrow edge margin and the entire facial outline, preserves broad negative space opposite the subject, and forbids centered placement. Both keep the neck, shoulders, and upper torso visible.
