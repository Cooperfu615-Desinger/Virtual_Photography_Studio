...
The phase-1 structural gate is `engine/fixedFramingDerivedPromptContract.test.js`, included in `npm run test:prompt-quality`. It verifies frozen serializable policy data, the exact current IDs and complete active/legacy partition, all target wardrobe and compatibility fixtures, both half-face sides, and the intentionally disconnected face/chest runtime baseline. The phase-2 infrastructure gate is `engine/fixedFramingDerivedPromptInfrastructure.test.js`; it verifies the frozen full-body preset, non-mutating derived context, `fullBody` projection, compatibility metadata, raw source-selection preservation, absence of the two future outputs, and exact full-body output bytes for deterministic normal, special-outfit, Character Card, and fixed-composition cases. Public Prompt text, output contracts, UI, random pools, Saved Cards, and DLL PIC Pro remain unchanged in phase 2.
