...
2026-09-13 窄範圍補充：四個已核准低機位上方場景來源由 `webapp/src/lib/engine/zImageUpperScene.js` 按既有 location ID 綁定，不更動 Locations 主資料；只供一般單人主 Z-Image 使用。維護與驗證邊界見 [上方場景 v2 首批規格](z-image-upper-scene-v2.md)，其餘新增／修改資料仍依本文件。
