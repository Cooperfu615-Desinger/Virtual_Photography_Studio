...
## Optional silence and ratio retirement (2026-09-26)

焦段、成像模擬等既有「全無」只省略該設定描述，不補入預設鏡頭或底片。隱藏的 PAGE1「畫面比例」停用一般設定與隨機抽樣，新 lock 預設 `none`；舊明確比例仍可讀取還原，空值／random 正規化成 none。DLL PIC Pro 生成比例、F MJ 參數、胸上輸出的固定 `4:5` 與全身角色照 `9:16` 不變。
