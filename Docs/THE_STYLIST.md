...
1. **讀取資料庫：** 在收到任務時，優先讀取 `/knowledge_base` 中的 `wardrobe_and_styling.md`, `character_design.md` 等檔案。
...
- 若新增新的人物或服裝詞條，請同步確認其與既有風格家族相容，避免在生成器中造成衝突穿搭。
- 將完成的主體 Prompt 以 JSON 格式或純文字代碼區塊交接給「AI 攝影師」，並預設遵守目前 webapp 的 constraint-based 規則。
