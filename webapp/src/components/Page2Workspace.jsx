...
const OUTPUT_COPY_LABELS = {
  ...
  headshot: '大頭照',
  'four-view': '四視圖',
  'full-body-reference': '全身 Reference',
};

const OUTPUT_COPY_ORDER = [
  'gpt',
  'headshot',
  'grok-z-image',
  'four-view',
...
export default function Page2Workspace({
  ...
  const orderedCopyOutputs = OUTPUT_COPY_ORDER
  ...
  return (
    <section className="page2-shell character-card-lab-shell">
      <section className="lock-panel page2-panel character-card-picker-workspace">
      ...
      <div className="character-card-lab-side-column">
        {activeCard ? (
          <section className="lock-panel page2-panel character-card-config-panel">
            <div className="control-section-title">角色設定與 PAGE1 匯入</div>
              ...
              <div className="character-card-output-actions">
                {orderedCopyOutputs.map((output) => (
                  <button
                    ...
                    title={`Copy ${output.label}`}
                  >
                    {OUTPUT_COPY_LABELS[output.id] || output.label}
                  </button>
                ))}
              </div>
