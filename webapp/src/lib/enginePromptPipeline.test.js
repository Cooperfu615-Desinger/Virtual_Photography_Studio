...
test('AI single-subject prompt orders eyewear and headphones before clothing', () => {
  ...
  assert.match(aiPrompt, /^Photorealistic editorial portrait\.[\s\S]*A 20s seductive stunning Japanese woman\./i);
...
test('Gpt duo mixed role outfits keep the other person wardrobe selections', () => {
  const locks = {
    ...
    topBColorId: optionId('topBColorId', '粉紅色'),
    pantsBId: optionId('pantsBId', '直筒牛仔褲'),
    bottomBColorId: optionId('bottomBColorId', '粉紅色'),
...
test('Gpt duo mixed preset and garment selections stay visible in prompt and summary', () => {
  const locks = {
    ...
    outfitPresetAPrimaryColorId: optionId('outfitPresetAPrimaryColorId', '寶藍色'),
    topBId: optionId('topBId', '襯衫'),
    pantsBId: optionId('pantsBId', '直筒牛仔褲'),
    shoesBId: optionId('shoesBId', 'Samba OG'),
    framingId: optionId('framingId', '全身鏡頭 (Full Body Shot)'),
...
test('Z-Image prompt remains natural language with blank-line paragraphs and AI uses a compact natural sentence', () => {
  ...
  assert.match(prompt.midjourneyPrompt, / A 20s seductive stunning Japanese woman\./);
